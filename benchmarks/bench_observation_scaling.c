/**
 * @file bench_observation_scaling.c
 * @brief Measure performance scaling with observation count
 */
...
int main(int argc, char** argv)
{
    ...
    double* observations;
    int N = 10000;
    int T_values[] = {100, 500, 1000, 5000, 10000, 50000, 100000};
    int num_tests = sizeof(T_values) / sizeof(T_values[0]);
    int max_T = T_values[num_tests - 1];
    int i;
    ...
    /* Generate data */
    x_true = (double*)malloc(max_T * sizeof(double));
    observations = (double*)malloc(max_T * sizeof(double));
    generate_data(x_true, observations, max_T, params.process_std, params.obs_std, 12345);
    ...
    for (i = 0; i < num_tests; i++) {
        run_test(N, T_values[i], &params, observations);
    }
